...
Page({
                ...
                if (console.log(e), 0 == e.data) c.setData({
                    ...
                    c.setData({
                        ...
                        success: function (e) {
                            ...
                            var a = e.data;
                            for (var t in a)
                                if (a[t].hidden1 = !0, 0 != a[t].money ? a[t].money1 = "（收费" + a[t].money + "元）" : a[t].money1 = "  免费",
    ...
    chooseImage2: function () {
        ...
        var t = this,
            e = t.data.imgArray;
        console.log(t.data);
        var o = wx.getStorageSync("uniacid"),
            a = 9 - e.length;
        0 < a && a <= 9 ? 
        app.uploadPictures(a)
        .then((e) => {
    ...
    formSubmit: function (e) {
        var a = this;
        if (console.log(e), console.log(a.data), null == a.data.inmoney) var t = 0;
        else t = Number(a.data.inmoney);
        var o = a.data.type;
        console.log(t + " " + o);
        ...
        var i = wx.getStorageSync("openid"),
            ...
            d = e.detail.value.address,
            p = (a.data.time,
                a.data.time1, a.data.time1),
            g = a.data.time,
            m = e.detail.value.vr_demo,
            f = e.detail.value.business_number,
            h = e.detail.value.business_notice,
            y = e.detail.value.textarea,
            v = a.data.store_info.coordinates,
            w = a.data.store,
            x = a.data.store2,
            _ = a.data.index,
            T = a.data.index_two,
            ...
            k = x[T],
            S = w[_].id;
        if (0 < x.length) var b = x[T].id;
        else b = "";
        console.log(w, x, D, k, S, b);
        var A = a.data.tel_code,
            z = a.data.num,
            q = (T = a.data.index_two, imgArray.join(",")),
            C = uploaded.join(",");
        a.data.city;
        ...
        if ("" == u ? P = "请输入商家名称" : "" == r ? P = "请输入关键字" : "" == d ? P = "请输入详细地址" : "" == f ? P = "请输入联系电话" : "00:00" == g ? P = "请输入营业开始时间" : "00:00" == p ? P = "请输入营业结束时间" : "" == h ? P = "请输入公告说明" : 0 == a.data.sms ? "" == z && (P = "请进行手机号验证") : 1 == a.data.sms && null == a.data.yz_code && (P = "请进行手机号验证"),
            ...
            if (null == s) {
                for (var N in R)
                    if (1 == R[N].checked)
                        if ("刷卡支付" == R[N].value) I = 1;
                        else if ("免费wifi" == R[N].value) j = 1;
                else if ("免费停车" == R[N].value) M = 1;
                else if ("禁止吸烟" == R[N].value) F = 1;
                else if ("提供包间" == R[N].value) U = 1;
                else if ("沙发休闲" == R[N].value) L = 1;
            } else
                for (var O = 0; O < s.length; O++)
                    if ("刷卡支付" == s[O]) I = 1;
                    else if ("免费wifi" == s[O]) j = 1;
            else if ("免费停车" == s[O]) M = 1;
            else if ("禁止吸烟" == s[O]) F = 1;
            else if ("提供包间" == s[O]) U = 1;
            else if ("沙发休闲" == s[O]) L = 1;
            if (1 == A)
                if (2 == a.data.store_info.time_over) app.util.request({
