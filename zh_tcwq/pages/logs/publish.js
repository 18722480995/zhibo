...
Page({
    ...
    imgArray1: function (e) {
        var t = this,
            s = wx.getStorageSync("uniacid"),
            o = this.data.imgarr1;
        _imgArray = [], console.log(o)
        app.uploadPictures(4 - o.length)
            .then((res) => {
                o = o.concat(res.tempFilePaths[0]), t.uploadimg({
        ...
        //         console.log(e.tempFiles[0].size)
        //         if (e.tempFiles[0].size > 102400) {
        //             wx.compressImage({
        //                 src: a[0], // 图片路径
        //                 quality: 10, // 压缩质量
        //                 success(res) {
    ...
    imgArray2: function (e) {
        var t = this,
            s = wx.getStorageSync("uniacid"),
            o = this.data.imgarr2;
        _imgArray2 = [], console.log(o),
        app.uploadPictures(8 - o.length)
        .then((a)=>{
            o = o.concat(a.tempFilePaths), t.uploadimg1({
                url: t.data.url + "app/index.php?i=" + s + "&c=entry&a=wxapp&do=Upload&m=zh_tcwq",
                path: o
            });
        })
    ...
    add: function (e) {
        ...
        for (var o in console.log(a), t[a].spec.push(s), t)
            for (var c in t[o].spec) t[o].spec[c].id = t[o].id;
    ...
    delete_spec_small: function (e) {
        var a = e.currentTarget.dataset.index,
            ...
            s = this.data.spec;
        for (var o in console.log(a), s[t].spec.splice(1, 1), s)
            for (var c in s[o].spec) s[o].spec[c].id = s[o].id;
        this.setData({
            spec: s
        });
    },
    ...
    formSubmit: function (e) {
        ...
        var t = e.detail.value.spec_name,
            s = e.detail.value.spec_num,
            o = e.detail.value.spec_price,
            c = e.detail.value.spec_freight,
            i = e.detail.value.spec_delivery,
            n = e.detail.value.goods_details,
            l = a.data.check_box,
            r = a.data.spec;
        for (var p in r)
            for (var d in r[p].spec) r[p].spec[d].spec_id = r[p].spec_id, r[p].spec[d].spec_name = r[p].text;
        console.log(r);
        var u = [];
        ...
        var w = "";
        if ("" == t) w = "商品名称不能为空";
        else if ("" == o) w = "商品价格不能为空";
        else if ("" == s) w = "商品数量不能为空";
        else if ("" == c) w = "商品运费不能为空";
        else if ("" == i) w = "发货说明不能为空";
        else if (0 == _imgArray.length) w = "还没有上传商品图片哦";
        else if (1 == a.data.add_spec)
            for (var D in u) "" == u[D].spec_name ? w = "没有选择规格" : "" == u[D].name ? w = "还有规格名字没输入" : "" == u[D].price && (w = "还有规格价格没输入");
        if ("" != w) wx.showModal({
            title: "提示",
            content: w,
            showCancel: !0,
            ...
            if (0 < _imgArray.length) var A = _imgArray.join(",");
            else A = "";
            if (0 < _imgArray2.length) var S = _imgArray2.join(",");
            else S = "";
